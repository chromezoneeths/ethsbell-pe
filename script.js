  ...
  if (response.dayOfWeek == "Saturday" || response.dayOfWeek == "Sunday") {
    ...
    for (var a = 1; a <= periodCount; a++) {
      ...
      for (var b = 0; b < Object.keys(period).length; b++) {
        ...
        cell.classList += "maincell";
        cell.innerHTML =
          Object.entries(period)[b][0] /*Teacher name*/ +
          ": " +
          Object.entries(period)[b][1] /*Location name*/;
  ...
  for (var j = 0; j < teacherArray.length; j++) {
    for (var k = 0; k < periodList.length; k++) {
      if (Object.values(data[j])[k + 1] !== "null") {
        Locations[periodList[k]][data[j].teachername] = Object.values(data[j])[
          k + 1
        ];
      }
    }
...
function search() {
  ...
  td = table.querySelectorAll("td.maincell");
